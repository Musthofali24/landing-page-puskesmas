...
const Staff = () => {
  ...
  // Data dokter dan tenaga kesehatan (bisa ditambah sesuai kebutuhan)
  const staffData = [
    // Slide 1
    [
      {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentSlide((prev) => {
        const next = (prev + 1) % staffData.length;
  ...
  const handleDragEnd = () => {
    ...
    if (movedBy < -20 && currentSlide < staffData.length - 1) {
      ...
      <div className="max-w-7xl px-4 mx-auto">
        ...
        <div className="mt-12 relative">
          <div className="overflow-hidden py-3">
            ...
            >
              {staffData.map((slideStaff, slideIndex) => (
                <div
                  key={slideIndex}
          ...
          <button
            onClick={() =>
              goToSlide(
                (currentSlide - 1 + staffData.length) % staffData.length
              )
            }
            className="absolute -left-15 top-1/2 transform -translate-y-1/2 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-700 text-gray-800 dark:text-white p-3 rounded-full transition-all duration-300 shadow-md z-10"
            aria-label="Previous slide"
          >
            <FaArrowLeft size={20} />
          </button>
          <button
            onClick={() => goToSlide((currentSlide + 1) % staffData.length)}
            className="absolute -right-15 top-1/2 transform -translate-y-1/2 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-700 text-gray-800 dark:text-white p-3 rounded-full transition-all duration-300 shadow-md z-10"
